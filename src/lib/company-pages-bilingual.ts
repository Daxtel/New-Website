export const aboutPageBilingual = {
  title: { en: 'About Streetshow Productions', ja: 'Streetshow Productionsについて' },
  intro: {
    en: 'Streetshow Productions is the Japan-entry, localization, and creative execution partner for premium brands that cannot afford a weak launch.',
    ja: 'Streetshow Productionsは、弱いローンチが許されないプレミアムブランドのための、日本市場進出・ローカライズ・クリエイティブ実行パートナーです。',
  },
  mission: {
    en: 'We help international companies adapt, position, and execute for Japan with the cultural precision, production discipline, and brand sensitivity this market demands.',
    ja: '国際企業が、この市場が求める文化的精度、制作規律、ブランド感度をもって、日本向けに適応・ポジショニング・実行できるよう支援します。',
  },
  positioningLabel: { en: 'Positioning', ja: 'ポジショニング' },
  positioning: {
    en: 'A premium Japan market entry, localization, and execution partner for high-end brands.',
    ja: 'ハイエンドブランド向けの、日本市場進出・ローカライズ・実行パートナー。',
  },
  operatingEdgeLabel: { en: 'Operating Edge', ja: '強み' },
  primaryVerticalLabel: { en: 'Primary Vertical', ja: '主力領域' },
  primaryVertical: { en: 'Luxury hospitality', ja: 'ラグジュアリーホスピタリティ' },
  coreCapabilityLabel: { en: 'Core Capability', ja: '中核能力' },
  coreCapability: { en: 'Japan market entry, localization, and execution', ja: '日本市場進出・ローカライズ・実行支援' },
  bestFitLabel: { en: 'Best Fit For', ja: '最適な対象' },
  ctaTitle: { en: 'Discuss Your Project', ja: 'プロジェクトについて相談する' },
};

export const contactPageBilingual = {
  title: { en: 'Discuss Your Project', ja: 'プロジェクトについて相談する' },
  subtitle: {
    en: 'Streetshow Productions works with selected brands and operators where strategic clarity, cultural nuance, and execution quality are the difference between a launch that converts and one that gets ignored.',
    ja: 'Streetshow Productionsは、戦略の明確さ、文化的な適合、実行品質が成果に大きく影響するブランドや事業者を支援します。',
  },
  qualification: {
    en: 'Best suited for Japan market entry, hospitality repositioning, premium campaign execution, and localization-led brand work.',
    ja: '日本市場進出、ホスピタリティ再構築、プレミアム施策実行、ローカライズ主導のブランド案件に最適です。',
  },
  reassurance: {
    en: 'We review each inquiry carefully and respond to serious requests promptly.',
    ja: '各お問い合わせを丁寧に確認し、真剣なご相談には速やかに対応します。',
  },
  processNote: {
    en: 'Initial calls are used to assess fit, market context, and the level of support required.',
    ja: '初回の打ち合わせでは、適合性、市場背景、必要な支援レベルを確認します。',
  },
  contactInformation: { en: 'Contact Information', ja: '連絡先' },
  emailLabel: { en: 'Email', ja: 'メール' },
  locationLabel: { en: 'Location', ja: '拠点' },
  directPrompt: { en: 'Prefer to speak directly? Book a strategic call.', ja: '直接相談したい場合は、戦略コールをご予約ください。' },
  fields: {
    en: ['Full Name', 'Work Email', 'Company', 'Website', 'Project Type', 'Estimated Budget', 'Timeline', 'Japan Scope', 'Project Details'],
    ja: ['お名前', '業務用メール', '会社名', 'Webサイト', 'プロジェクト種別', '想定予算', 'スケジュール', '日本市場での範囲', 'プロジェクト詳細'],
  },
};
