    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: {
          ...
          ja: [
            '難しいことは何もありません。ただ、御社のローンチ資料に書かれたスケジュールより遅いというだけです。私たちが目にする遅延のほとんどは、事務手続きに見えて実は前工程に依存していた、という一点にさかのぼります。',
            'カレンダーを決めるのは、次の4つの問いです。',
            '輸入者は誰か。日本に法人格を持つ誰かが、輸入される商品に責任を負う必要があります。現地法人を設立しないのであれば、それは販売代理店、商社、あるいは輸入代行業者です。これは事務手続きではありません。価格決定権、チャネルとの関係、そして3年目にパートナーを替えられるかどうかを左右します。早く動きたいという理由で初月にこの権利を手放し、その後何年もかけて取り戻そうとするブランドは珍しくありません。',
            ...
            '消費税はどう扱われ、請求書はインボイス制度に対応しているか。適格請求書等保存方式は、取引先が支払った消費税をどう処理できるかに影響します。登録や請求書の様式を誤れば、卸先が想定外のコストを負担することになります。日本のバイヤーとの関係の始め方としては、最悪の部類です。',
      ...
      },
      {
        ...
        paragraphs: {
          ...
          ja: [
            ...
            '比較対象は誰か。日本の買い手は、御社をグローバルの競合とは比べません。その市場で同じ棚、同じ価格帯、同じカテゴリを占めているものと比べます。それが実際に誰なのかを把握してください。多くの場合、カテゴリ担当が名前も聞いたことのない国内ブランドです。',
            ...
            '変えてはいけないのは、御社が選ばれる理由そのものです。現地市場に似るまでブランドを寄せていくのは[ローカライズ](/japan-market-entry)ではなく、消失です。日本の買い手が、自分たちをより理解している既存プレーヤーではなく御社を選ぶ理由を、それは取り去ってしまいます。',
          ],
        },
      },
      {
        ...
        paragraphs: {
          ...
          ja: [
            ...
            'モール出店。楽天やAmazon.co.jpは、即座に大きな流通量の前に立たせてくれます。その代償として、比較しやすいように設計されたページの上で戦うことになります。そこでのビジュアルの作法と情報量は、欧米のECとは似ても似つきません。国際基準で見れば清潔でプレミアムに映る店舗ページが、情報の詰まった国内競合のページに負けることは珍しくありません。日本の買い手は、その情報量を「ちゃんとしている」と読むからです。',
            '卸・小売。適切な店に置かれることは、お金では簡単に買えない信頼のシグナルであり、オンラインでは決して御社を見つけない客層に届きます。同時に、時間がかかります。日本のバイヤーは長い周期で評価し、一貫性を求め、3年後もこのブランドが残っているかを見ています。最初の商談は、売り込みではなく関係の入り口として扱ってください。',
            'ディストリビューター。最短で市場に入れる経路であり、最も選択肢を失う経路でもあります。良い相手は、1年では築けない関係を持ってきてくれます。悪い相手は、御社と自社の顧客のあいだの壁になり、初月に交わした独占条項がその壁を支え続けます。この道を選ぶなら、条件より先に出口を交渉してください。',
      ...
      },
      {
        ...
        paragraphs: {
          ...
          ja: [
            ...
            '敬体と距離感。日本語は、話し手と受け手の関係を文法そのものに埋め込みます。英語では温かく率直なブランドボイスも、日本語では丁寧さの水準を選ばざるを得ず、その選択が「御社が顧客をどう見ているか」を伝えてしまいます。どちらに外しても、なれなれしいか、よそよそしいかになります。中立という選択肢はありません。',
            '展開の順序。日本の広告は、主張する前に文脈を置くのが一般的です。欧米のクリエイティブの多くは逆で、主張から入り、後から支えます。その構造のまま流すと、冒頭のカットが自信ではなく押しの強さとして読まれます。',
            '情報量。日本のオーディエンスは、欧米のオーディエンスが許容するより多くの情報を画面上・誌面上で受け入れ、しばしば期待します。1枚の画と3語まで削ぎ落としたクリエイティブは、洗練ではなく中身が薄いと読まれることがあります。',
