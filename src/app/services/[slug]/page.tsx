...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const svc = service as typeof service & { metaTitle?: string; metaDescription?: string };
  const title = svc.metaTitle || service.title;
  const description = svc.metaDescription || service.description;
...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const svcFaqs = (service as typeof service & { faq?: { q: string; a: string }[] }).faq;
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
            ...
            <div className="flex flex-col justify-end">
              <h2 className="text-2xl font-semibold text-[#D4AF37]/90 md:text-3xl">{service.headline}</h2>
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-2 lg:gap-12">
            <div className="border border-[#D4AF37]/10 bg-[#141414] p-8">
              <h2 className="text-xl font-bold uppercase tracking-[0.15em] text-[#D4AF37] md:text-2xl">{service.whyTitle}</h2>
              <p className="mt-4 text-base leading-relaxed text-white/65 md:text-lg">{service.whyBody}</p>
            </div>
            <div className="border border-[#D4AF37]/10 bg-[#141414] p-8">
              <h2 className="text-xl font-bold uppercase tracking-[0.15em] text-[#D4AF37] md:text-2xl">{service.mistakesTitle}</h2>
                  ...
                  return (
                    <Link key={project.slug} href={`/work/${project.slug}`} className="group block overflow-hidden bg-[#141414] transition-all hover:bg-[#D4AF37]/5 hover:scale-[1.02]">
                      ...
                      <div className="p-8">
                        ...
                        <h4 className="mt-3 text-2xl font-semibold text-[#D4AF37]">{project.title}</h4>
